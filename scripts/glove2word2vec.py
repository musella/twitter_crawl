...
def glove2word2vec(glove_vector_file, output_model_file):
    ...
    check = ["#machinelearning","#datascience","#ai","#robotics"]
    ## logger.info('Most similar to #machinelearning are: %s' % model.most_similar(positive=['#machinelearning'], topn=10))
    for word in check:
        logger.info('Most similar to %s are: %s' % (word, model.most_similar(positive=[word], topn=10)) )
    # logger.info('Similarity score between #machinelearning and #ai is %s ' % model.similarity('#machinelearning', '#ai'))
